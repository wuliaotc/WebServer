...
namespace reactor {
    namespace detail {
        ...
        struct Comp {
            ...
            bool
            operator()(const Transition &lhs, const Transition &rhs) const {
                if (compareGmt)
...
namespace reactor {
    namespace detail {
        ...
        const Localtime *
        findLocaltime(const TimeZone::Data &data, Transition sentry,
                      Comp comp) {
            const Localtime *local = NULL;

            if (data.transitions.empty() ||
                comp(sentry, data.transitions.front())) {
...
struct tm TimeZone::toLocalTime(time_t seconds) const {
    ...
    const detail::Localtime *local = findLocaltime(data, sentry,
                                                   detail::Comp(true));
