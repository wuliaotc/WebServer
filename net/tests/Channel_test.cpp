...
public:
    ...
    void start() {
        ...
        int ret = ::timerfd_settime(timerfd_, 0 /* relative timer */, &spec,
                                    NULL);
        if (ret) {
...
private:
    void handleRead() {
        ...
        if (::read(timerfd_, &howmany, sizeof(howmany)) != sizeof(howmany))
            LOG_ERROR << "read timefd err";
        if (cb_)
            cb_();
    }
    ...
    static struct timespec toTimeSpec(double seconds) {
        ...
        int64_t nanoseconds = static_cast<int64_t>(seconds *
